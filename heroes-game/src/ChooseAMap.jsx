...
const ChooseAMap = () => {
    ...
    const invisibleNameState = `absolute top-4 right-1/2 translate-x-1/2 text-white text-lg invisible uppercase`
    const visibleNameState = `absolute top-4 right-1/2 translate-x-1/2 text-white text-lg visible uppercase`
    ...
    return (
            ...
            <div className='box- flex justify-center gap-2 w-full my-5'>
                    ...
                    <span className={NyImgState}>
                        <img src={ny} onClick={()=>active(setNyImgState, setNyNameState)} alt="" className='object-cover w-full h-maps rounded-lg'/>
                        <span className={NyNameState}>NEW YORK</span>
                    </span>

                    <span className={HiddenLeafImgState}>
                        <img src={hiddenLeaf} onClick={()=>active(setHiddenLeafImgState, setHiddenLeafNameState)} alt="" className='object-cover w-full h-maps rounded-lg'/>
                        <span className={HiddenLeafNameState}>Hidden Leaf</span>
                    </span>

                    <span className={GothamImgState}>
                        <img src={gotham} onClick={()=>active(setGothamImgState, setGothamNameState)} alt="" className='object-cover w-full h-maps rounded-lg'/>
                        <span className={GothamNameState}>Gotham city</span>
                    </span>

                    <span className={MetropolisImgState}>
                        <img src={metropolis} onClick={()=>active(setMetropolisImgState, setMetropolisNameState)} alt="" className='object-cover w-full h-maps rounded-lg'/>
                        <span className={MetropolisNameState}>Metropolis</span>
                    </span>

                    <span className={TownsvilleImgState}>
                        <img src={townsville} onClick={()=>active(setTownsvilleImgState, setTownsvilleNameState)} alt="" className='object-cover w-full h-maps rounded-lg'/>
